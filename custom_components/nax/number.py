...
class NaxToneGeneratorFrequencyNumber(NaxEntity, NumberEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{format_mac(mac_address)}_tone_generator_frequency"
...
class NaxZoneTestToneVolumeNumber(NaxEntity, NumberEntity):
    ...
    def __init__(
        ...
        self._zone_output_key = zone_output_key
        self._attr_unique_id = (
            f"{format_mac(mac_address)}_{zone_output_key}_test_tone_volume"
        )
        self.entity_id = f"number.{format_mac(mac_address)}_{zone_output_key.lower()}_test_tone_volume"
        self._attr_icon = "mdi:sine-wave"
        self._attr_native_unit_of_measurement = "%"
